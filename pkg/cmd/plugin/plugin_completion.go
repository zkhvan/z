...
func SetupPluginCompletion(cmd *cobra.Command, args []string) {
	...
	if len(args) > 0 {
		...
		if !found {
			...
			//
			// When using a plugin, the z global flags are not supported.
...
func registerPluginCommands(z *cobra.Command, list bool) (cmds []*cobra.Command) {
	...
	for _, plugin := range plugins {
		...
		// Plugins are named "z-<name>" or with more - such as
		// "z-<name>-<subcmd1>..."
		...
		// Iterate through all segments, for z-my_plugin-sub_cmd, we will end up with
		...
		parentCmd, remainingArgs, _ := z.Find(args)
		if parentCmd == nil {
			parentCmd = z
		}
...
//
// z's plugin completion logic supports Cobra's ShellCompDirective system.  This means a plugin
...
//
// The completion executable should be named z_complete-<plugin>.  For example, for a plugin
// named z-get_all, the completion file should be named z_complete-get_all.  The completion
...
func lookupCompletionExec(pluginName string) (string, bool) {
	...
	// For example, convert z-get_all to z_complete-get_all
