...
func NewCmdDown(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "down [<instance>]",
		Short: "Bring a workspace's runtime down",
		Long: heredoc.Doc(`
			Invoke the instance's runtime/down script, which tears its container
			or VM down.
...
func (opts *Options) Run(ctx context.Context) error {
	...
	code, err := svc.Runtime(ctx, name, workspace.RuntimeDown, nil)
	if err != nil {
		return err
	}
	if code != 0 {
