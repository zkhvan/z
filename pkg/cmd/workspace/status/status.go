...
func NewCmdStatus(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "status [<instance>]",
		Short: "Report a workspace's runtime status",
		Long: heredoc.Doc(`
			Invoke the instance's runtime/status script and report its output
			and exit code.
			...
			environment. Its exit code becomes z's, so a non-zero status (for
			example "not running") is surfaced rather than treated as a z error.
...
func (opts *Options) Run(ctx context.Context) error {
	...
	code, err := svc.Runtime(ctx, name, workspace.RuntimeStatus, nil)
	if err != nil {
		return err
	}
	if code != 0 {
